...
| C-013 | `docs/index.html` is the only separate threshold page and `docs/home.html` now carries the primary single-page exhibition route.                                                    | `PROMPT_LOG.md`, `docs/index.html`, `docs/home.html`                | Verified           | `docs/index.html` links into `home.html`; `docs/home.html` contains anchored sections for opening, chronology, companion rooms, and highlights.                       | Recheck if the museum returns to multi-page primary navigation.         |
| C-014 | The `Example/` folder remains an isolated prototype space and is not the live museum surface.                                                                                       | `PROMPT_LOG.md`, repository structure                               | Verified           | Active public pages live under `docs/`; the `Example/` folder contains separate prototype files and is not part of the Pages entry route.                             | Keep prototype work sandboxed unless explicitly promoted into `docs/`.  |
| C-015 | The live museum design now intentionally merges prototype-style single-page staging with curator-led chronology, object evidence, and companion-room logic.                         | `PROMPT_LOG.md`, `docs/home.html`, `AGENTS.md`                      | Partially verified | The repository shows the implemented hybrid structure and documented design decision, but the curatorial success of the merge still depends on ongoing visual review. | Continue reviewing the live route as changes accumulate.                |
| C-016 | The live museum's primary object, portrait, and prop surfaces now use locally stored JPEG assets instead of the earlier placeholder SVGs.                                           | `PROMPT_LOG.md`, `docs/home.html`, `docs/media/`                    | Verified           | Active live pages reference `.jpeg` files in `docs/media/` for comic covers, portraits, and exhibit objects; the earlier placeholder SVGs are no longer present.      | Recheck if any surface returns to placeholder artwork later.            |
| C-017 | The live header now uses four primary exhibit-map tabs for the chronology spine and a smaller secondary row for companion rooms.                                                    | `PROMPT_LOG.md`, `docs/home.html`, `docs/styles.css`                | Verified           | Header markup shows `Opening hall`, `Room 1`, `Room 2`, and `Room 3` in the primary tab tray, with `Cast`, `Proof`, and `Wall` in the secondary chip row.             | Recheck if navigation is redesigned again.                              |
| C-018 | `docs/media/` has been pruned so it contains only assets still referenced by the live museum's HTML/CSS surfaces.                                                                   | `PROMPT_LOG.md`, `docs/media/`, `docs/**/*.{html,css}`              | Verified           | Current media inventory matches the set of assets referenced from `docs/` HTML/CSS, with duplicates and obsolete placeholders removed.                                | Recheck after future media imports or redesign work.                    |
